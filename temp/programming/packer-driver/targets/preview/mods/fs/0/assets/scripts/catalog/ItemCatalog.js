    ...
    execute: function () {
      ...
      _export("ItemCatalog", ItemCatalog = (_dec = ccclass('ItemCatalog'), _dec2 = property({
        ...
        rebuild() {
          ...
          for (var i = 0; i < this.itemPrefabs.length; i++) {
            var prefab = this.itemPrefabs[i];
          ...
          var count = Math.min(this.aliasIds.length, this.aliasPrefabs.length);

          for (var _i = 0; _i < count; _i++) {
            var id = this.aliasIds[_i];
            var _prefab = this.aliasPrefabs[_i];

            if (id && _prefab) {
              this._map.set(id, _prefab);
            }
          }
        }
