    ...
    execute: function () {
      ...
      _export("SpatialHashGrid", SpatialHashGrid = class SpatialHashGrid {
        ...
        insert(item, position) {
          this.remove(item);
          var key = this.getKey(position.x, position.z);

          var cell = this._cells.get(key);
        ...
        remove(item) {
          var key = this._itemKeys.get(item);
        ...
        query(position, radius, out) {
          out.length = 0;
          var minX = Math.floor((position.x - radius) / this._cellSize);
          var maxX = Math.floor((position.x + radius) / this._cellSize);
          var minZ = Math.floor((position.z - radius) / this._cellSize);
          var maxZ = Math.floor((position.z + radius) / this._cellSize);

          for (var x = minX; x <= maxX; x++) {
            for (var z = minZ; z <= maxZ; z++) {
              var cell = this._cells.get(x + ":" + z);
