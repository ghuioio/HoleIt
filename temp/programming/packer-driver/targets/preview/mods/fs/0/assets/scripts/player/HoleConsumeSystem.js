    ...
    execute: function () {
      ...
      _export("HoleConsumeSystem", HoleConsumeSystem = (_dec = ccclass('HoleConsumeSystem'), _dec2 = property({
        ...
        captureNearbyDynamicItems() {
          ...
          var holeRadius = this.holeSize.radius;
          var holeLevel = this.holeSize.level;
          ...
          for (var i = 0; i < this._dynamic.length; i++) {
            ...
            var allowedRadius = Math.max(0.02, holeRadius - item.consumeRadius - this.rimPadding);
        ...
        updateFallingItems() {
          for (var i = this._falling.length - 1; i >= 0; i--) {
            var item = this._falling[i];
        ...
        buildPullVelocity(itemPosition, out) {
          var dx = this._holePos.x - itemPosition.x;
          var dz = this._holePos.z - itemPosition.z;
