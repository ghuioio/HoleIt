    ...
    execute: function () {
      ...
      _export("JoystickInput", JoystickInput = (_dec = ccclass('JoystickInput'), _dec2 = property({
        ...
        updateFromTouch(event) {
          ...
          var x = this._localPos.x;
          var y = this._localPos.y;
          var length = Math.sqrt(x * x + y * y);

          if (length > this.radius && length > 0.0001) {
            var scale = this.radius / length;
            x *= scale;
            y *= scale;
          }
          ...
          var normalizedX = this.radius > 0 ? x / this.radius : 0;
          var normalizedY = this.radius > 0 ? y / this.radius : 0;
          var normalizedLength = Math.sqrt(normalizedX * normalizedX + normalizedY * normalizedY);
          ...
          var invLength = normalizedLength > 1 ? 1 / normalizedLength : 1;
